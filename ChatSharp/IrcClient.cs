...
namespace ChatSharp
{
    ...
    /// An IRC client.
    /// </summary>
    public partial class IrcClient //i (bl4ckscor3) changed this class to adapt to bl4ckb0t's l10n and ignoring features
    {
        /// <summary>
        ...
        private void ConnectComplete(IAsyncResult result)
        {
            try
            {
                ...
                if (UseSSL)
                {
                    ...
                    ((SslStream)NetworkStream).AuthenticateAsClient(ServerHostname, null, SslProtocols.Tls12, true);
                }
        ...
        internal void OnPrivateMessageRecieved(PrivateMessageEventArgs e)
        {
            if (PrivateMessageRecieved != null)
			{
				try
				{
					if(!((List<string>)Data.information["ignoredUsers"]).Contains(e.PrivateMessage.User.Nick))
        ...
        internal void OnChannelMessageRecieved(PrivateMessageEventArgs e)
        {
			if (ChannelMessageRecieved != null)
			{
				try
				{
					if(!((List<string>)Data.information["ignoredUsers"]).Contains(e.PrivateMessage.User.Nick))
					{
						Thread.CurrentThread.CurrentUICulture = CultureInfo.CreateSpecificCulture(((Dictionary<string, string>)Data.information["langs"])[e.PrivateMessage.Source]);
